...
extension TodosAPI {
        ...
        BaseListResponse<Todo>
    {
        ...
        do {
            ...
            if let myApiError = error as? ApiError {
                throw myApiError
            }
...
//MARK: - Async Retry
// Task 에서 Failure 타입이 Error 인 경우에 아래 retry 함수를 사용할 수 있다.
    ...
    static func retry(retryCount:Int = 1,
        ...
        return Task {
            ...
            for _ in 0...retryCount {
                do {
                    // Success는 Task 블록 안에 있는 타입
                    let result = try await asyncWork()
                    print("result : \(result)")
                    // asyncWork가 통과되면 result에 Success가 담기게 된다.
                    return result
                } catch {
                    print("error: \(error)")
